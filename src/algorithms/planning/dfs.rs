...
mod tests {
    ...
    fn returns_error_when_start_does_not_exist() {
        ...
        let result = Dfs::execute(&graph, 0, goal);
    ...
    fn returns_error_when_goal_does_not_exist() {
        ...
        let result = Dfs::execute(&graph, start, 1);
    ...
    fn returns_error_when_path_does_not_exist() {
        ...
        let result = Dfs::execute(&graph, start, goal);
    ...
    fn solves_cyclic_graph() {
        ...
        match Dfs::execute(&graph, 0, 3) {
            Ok(path) => assert_eq!(VecDeque::from([0, 2, 3]), path),
            Err(err) => panic!("Expected a valid path, got error: {:?}", err),
        }

        match Dfs::execute(&graph, 1, 2) {
            Ok(path) => assert_eq!(VecDeque::from([1, 3, 2]), path),
            Err(err) => panic!("Expected a valid path, got error: {:?}", err),
        }
    }
    ...
    fn solves_disconnected_graph() {
        ...
        match Dfs::execute(&graph, 0, 1) {
            Ok(path) => assert_eq!(VecDeque::from([0, 1]), path),
            Err(err) => panic!("Expected a valid path, got error: {:?}", err),
        }

        match Dfs::execute(&graph, 3, 2) {
    ...
    fn finds_feasible_path() {
        ...
        // Order matters here, dfs will explore branche going from 2
        // even if a shorter path through 1 exists
        graph.add_edge(UnweightedEdge::new(0, 1));
        graph.add_edge(UnweightedEdge::new(0, 2));

        graph.add_edge(UnweightedEdge::new(1, 4));

        graph.add_edge(UnweightedEdge::new(2, 1));
        graph.add_edge(UnweightedEdge::new(2, 3));
        ...
        graph.add_edge(UnweightedEdge::new(4, 0));

        match Dfs::execute(&graph, 0, 4) {
            Ok(path) => assert_eq!(VecDeque::from([0, 2, 3, 4]), path),
            Err(err) => panic!("Expected a valid path, got error: {:?}", err),
        }
    }
}
